...
import { mapDepartmentLabel } from '@/utils/networking';
import type { NetworkingParticipant } from '../services/getNetworkingParticipantsList';
import { buildParticipantTagItemsN } from '@/utils/networking/participantLabels';

type ModifyRepresentativeCardProps = {
  participant: NetworkingParticipant;
  isSelected: boolean;
  onSelect: () => void;
};

export const ModifyRepresentativeCard = ({
  participant,
  isSelected,
  onSelect,
}: ModifyRepresentativeCardProps) => {
  const theme = useTheme();
  const tagItems = buildParticipantTagItemsN(participant, {
